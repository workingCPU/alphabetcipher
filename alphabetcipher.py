...
class alphaDecrypt() :
	...
	def analyze(me, cstmCiph = "") :
		if cstmCiph != "" :
			...
			cipher = me.cipher # otherwise use default ciph.
		...
		dist = p-65 # ascii code to A=1 format.
		...
		print(f"Predicted key: {pKey} ; Distance: {dist}\n")

		return [dist, pKey] # pKey: not for mono but poly, e.g.
	...
	def trigraph(me, cstmCiph = "") :
		...
		wordList = cipher.split(" ") # get word-list.
	...
	def decrypt(me, char, dist) :
		clearC = lambda c, k : chr((ord(c) - k - 65) % 26 + 65)  # clear Char.
	...
	def decryptPoly(me, pKeyLen = 6) :
		...
		print(clear)
		triRes = me.trigraph() # no args, just testing.
